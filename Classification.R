...
setwd(ABS_PATH)
source('./utils.R')
ds = ds.init(DATASET_FILENAME, Z_LABEL, PREDICTORS_NUMBER)
...
glm.inspect
glm.byFormulaChunks
glm.inspectInteractionMatrix
glm.bestSubsetSelection
glm.bestSubsetsByPredictorsNumber


#==================== REGRESSION WITHOUT INTERACTIONS ====================

baseModel=glm.byIndices(ds, -1)
glm.inspect(baseModel, 5)
...
dependencyModel = glm.byFormulaChunks(ds, possibleDependencies)
...
interactionMatrix = glm.inspectInteractionMatrix(ds, default=baseRSquared, showHeatmap = T)
...
bestSubsets = glm.bestSubsetSelection(ds, relationships=possibleRelationships, nMSE=10, folds=5, verbose=T, method="exhaustive")
...
bestSubsets = glm.bestSubsetsByPredictorsNumber(ds, relationships=possibleRelationships, nMSE=10, folds=5, nPredictors=N_PREDICTORS_TO_INSPECT, nSubsets=10, verbose=T)
ds.prettyPlot(bestSubsets$MSE, xlab="Number of predictors", ylab="CV test MSE", title="5-fold cross-validation Test MSE")
...
bestSubsets = glm.bestSubsetSelection(ds, relationships=possibleRelationships, nMSE=10, folds=5, method="forward", nvmax=8, verbose=T)
bestSubset = bestSubsets$model[[which.min(bestSubsets$MSE)]]
